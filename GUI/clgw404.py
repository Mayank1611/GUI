...
def choise1():
    if(source_value.get()==1):
        root.configure(background='red')
    elif(source_value.get()==2):
...
rb1_box=Radiobutton(root,text="Delhi",variable=source_value,value=1,width=20,command=choise1).grid(row=4,column=3)
rb2_box=Radiobutton(root,text="Mumbai",variable=source_value,value=2,width=20,command=choise1).grid(row=4,column=4)
